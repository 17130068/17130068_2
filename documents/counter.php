<?php
$c_version="ezgenerator counter 5.1.1";
/*
...
$c_db_dir=(isset($rel_path)?$rel_path:'../').$f->db_folder;
...
{
	global $c_db_details_fname,$c_db_totals_fname,$f;
	...
	else
	{
		...
		else
		{
			...
			if($rss_flag)
			{
				...
				{
					$old=Formatter::GFS($file_contents,'<rss_'.$page_id.'>','</rss_'.$page_id.'>');
				...
				if(isset($_GET['fl']))
				{
					...
					{
						$old=Formatter::GFS($file_contents,'<flrss_'.$page_id.'>','</flrss_'.$page_id.'>');
			...
			else
			{
				...
				{
					$old=Formatter::GFS($file_contents,'<l_'.$page_id.'>','</l_'.$page_id.'>');
				...
				{
					$old=Formatter::GFS($file_contents,'<loads>','</loads>');
					$total_loads=$old+1;
				...
				if(strpos($file_contents,'<unique>')!==false) $total_unique=Formatter::GFS($file_contents,'<unique>','</unique>');
				...
				{
					$old=Formatter::GFS($file_contents,'<first>','</first>');
				...
				{
					$old=Formatter::GFS($file_contents,'<returning>','</returning>');
...
{
	global $f,$c_version,$c_db_settings_fname,$c_db_totals_fname,$C_MAX_VISIT_LENGHT,$C_DISPLAY,$C_GRAPHICAL,$C_UNIQUE_START_COUNT, $C_LOADS_START_COUNT, $C_NUMBER_OF_DIGITS,$page_id;

	$action_id=isset($_REQUEST['action'])?Formatter::stripTags($_REQUEST['action']):'hit';
	$timestamp=time();
	...
	$settings=File::read($c_db_settings_fname);
	$display=Formatter::GFS($settings,'<display>','</display>'); if($display=='') $display=$C_DISPLAY;
	$loads_start_count=Formatter::GFS($settings,'<loads_start_value>','</loads_start_value>'); if($loads_start_count=='') $loads_start_count=$C_LOADS_START_COUNT;
	$unique_start_count=Formatter::GFS($settings,'<unique_start_value>','</unique_start_value>'); if($unique_start_count=='') $unique_start_count=$C_UNIQUE_START_COUNT;
	$cookie_suffix=Formatter::GFS($settings,'<counter_cookie_suffix>','</counter_cookie_suffix>');
	$rss_count=Formatter::GFS($settings,'<rss_count>','</rss_count>'); if($rss_count=='') $rss_count='0';
	...
	elseif($action_id=="rss")
	{
		if($rss_count=='1')
		{
			$stat_detailed=Builder::detailedStat($timestamp,$page_id,$uniq_flag,$firsttime_flag,true);
	...
	elseif($action_id=="hit")
	{
		...
		{
			$max_visit_len=Formatter::GFS($settings,'<max_visit_len>','</max_visit_len>'); if($max_visit_len=='') $max_visit_len=$C_MAX_VISIT_LENGHT;
			...
			$stat_detailed=Builder::detailedStat($timestamp, $page_id, $uniq_flag, $firsttime_flag);
		...
		{
			$counter_stat=File::readTaggedData($c_db_totals_fname,'totals');
			...
			$counts=Formatter::GFS($counter_stat,'<'.$tag.'>','</'.$tag.'>');
			settype($counts,"integer");
			$counts+=$add;
		}
		if($visible)
		{
			$number_of_digits=Formatter::GFS($settings,'<number_digits>','</number_digits>'); if($number_of_digits=='') $number_of_digits=$C_NUMBER_OF_DIGITS;
			$graphical=Formatter::GFS($settings,'<graphical>','</graphical>');
			if($graphical=='') $graphical=$C_GRAPHICAL;
			$direct=isset($_REQUEST['d']);

			if($graphical=='1')
			{
				$id=Formatter::GFS($settings,'<size>','</size>'); if($id=='') $id='1';
				$dir_pr=(isset($_REQUEST['root']) && ($_REQUEST['root']=='true'))?'':'../';
				$ipath=$dir_pr.'ezg_data/c'.intval($id).'.gif';
				$wh=explode('|',$f->counter_images[$id-1]);
